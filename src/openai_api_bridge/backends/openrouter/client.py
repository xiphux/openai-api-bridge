...
def extract_image_data_urls(response: dict[str, Any]) -> list[str]:
	...
	if not isinstance(message, dict):
		raise UpstreamError(
			"OpenRouter response choices[0].message is missing or malformed"
		)
	...
	if not urls:
		raise UpstreamError(
			"OpenRouter response carried an images array but no usable URLs"
		)
	return urls
...
async def fetch_image_bytes(
	...
	if url.startswith("data:"):
		...
		if not header.endswith("base64"):
			raise UpstreamError(
				"OpenRouter returned a non-base64 data URL — unsupported encoding"
			)
		...
		try:
			...
			raise UpstreamError(
				f"OpenRouter data URL contained undecodable base64: {e}"
			) from e
	...
	try:
		...
		raise UpstreamError(
			f"OpenRouter image fetch failed for {url}: {e}"
		) from e
